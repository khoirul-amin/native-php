...
<div class="main-content" id="panel">
  ...
  <div class="container-fluid mt--6">
    <div class="row">
      <div class="col-xl-12">
        <button type="button" class="btn btn-success mb-2" data-toggle='modal' data-target='#modalForm'>
              ...
              <tbody>
                ...
                while ($pelanggan = mysqli_fetch_array($result)) { ?>
                  <tr>
                    ...
                    <td>
                      <button type='button' class='btn btn-primary btn-sm' onclick='updateInformasi(<?= json_encode($pelanggan) ?>)' data-toggle='modal' data-target='#modalForm'><i class='fas fa-pencil-alt'></i></button>

                      <button type='button' class='btn btn-danger btn-sm' onclick="hapusData(<?= $pelanggan['id'] ?>)"><i class='fas fa-trash'></i></button>
                    </td>
                  </tr>
                <?php }; ?>
              </tbody>
...
<script>
  ...
  function hapusData(id) {
    Swal.fire({
      ...
      if (result.value) {
        // $.get(`../function/hapus_user.php?id=${id}`);
        window.location.href = `../function/hapus_user.php?id=${id}&halaman=karyawan`;
        // $.ajax({
        //   type: "GET",
